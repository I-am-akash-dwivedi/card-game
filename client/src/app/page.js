...
const socket_url =
  process.env.SOCKET_URL || "https://card-game-server-lxj5.onrender.com";
...
export default function Home() {
  ...
  socket.on("all-player-details", (all_player_details) => {
    ...
    Object.entries(all_player_details).forEach(
      ([player_id, thisPlayerDetail]) => {
  ...
  useEffect(() => {
    const nextRound = () => {
      ...
      Object.entries(playerDetails).forEach(([player_id, thisPlayerDetail]) => {
        ...
        thisPlayerDetail.backlog +=
          thisPlayerDetail.made - thisPlayerDetail.to_make;
        thisPlayerDetail.made = 0;
        const toMakeIndex = hands_to_make.indexOf(thisPlayerDetail.to_make);
        let to_make_for_this_round =
          hands_to_make[(toMakeIndex + 1) % hands_to_make.length];
    ...
    for (const thisTurnCard of thisTurnCards) {
      ...
      playerDetails[player_id].cards = playerDetails[player_id].cards.filter(
        (card) => card !== thisTurnCard
      );
      playerDetails[player_id].card_used.push(thisTurnCard);
    }
    if (
      thisTurnCards.length === player_ids.length &&
  ...
  const handleCardClick = (selectedCard) => {
    if (playerId === activePlayer) {
      if (!isValidCard(selectedCard)) {
        ...
        mineDetails.cards = mineDetails.cards.filter(
          (card) =>
  ...
  return (
    <div className="bg-blue-100 min-h-screen py-8 px-4">
      ...
      {!nameSubmitted ? (
        ...
        !isGameStarted && <Greeting name={name} />
      )}

      {thisGameSuit && (
        <div className="flex justify-center">
          <span className="fixed top-2 text-white bg-gradient-to-r from-purple-500 to-pink-500 focus:outline-none font-medium rounded-lg px-5 py-2.5 text-center text-sm animate-pulse cursor-pointer z-50">
            {playerDetails[activePlayer].name}&apos;s Turn
          </span>
        </div>
      )}
      ...
      {players.length > 0 && <PlayersInRoom players={players} />}
                ...
                {thisGameSuit && (
                  <div className="flex flex-col text-2xl justify-center items-center max-h-full m-2">
                    ...
                    <span className="text-3xl">
                      {suits[thisGameSuit]} ({thisGameSuit})
                    </span>
                  </div>
                )}
              </div>
            )}
            <div className="game-area mb-4">
              {thisTurnCards.length > 0 && (
                <div className="flex justify-center flex-wrap gap-4 mx-4">
                  {thisTurnCards.map((card, index) => (
                    <div
                      key={index}
                      className="w-1/4 sm:w-1/6 md:w-1/12 lg:w-1/12 xl:w-1/12"
                    >
      ...
      {isGameStarted && (
        <div
          ...
          className="bottom-0 right-0 p-4 flex flex-col-reverse items-end w-full"
        >
          <button onClick={toggleChat}>
            {isChatOpen ? (
              ...
              <div className="relative">
                <span className="bg-red-500 text-white px-2 py-1 rounded-full text-xs absolute top-0 right-0 transform translate-x-1/2 -translate-y-1/2">
                  {messages.length - msgSeenCount}
                </span>
                <div className="text-white bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 font-semibold py-2 px-4 rounded-lg">
                  Open Chat
                </div>
                  ...
                  messages.map((msg, index) => (
                    <div key={index}>
                      {msg.player_id === playerId ? (
                        <div className="max-w-2/3 w-fit py-1 px-4 m-1 flex flex-col ml-auto justify-end rounded text-white bg-gradient-to-r from-cyan-500 to-blue-500">
                          <p className="text-xs italic">{msg?.name}</p>
                          <p>{msg.msg}</p>
                        </div>
                      ) : (
                        <div className="max-w-2/3 w-fit py-1 px-4 m-1 rounded text-white bg-gradient-to-br from-pink-500 to-orange-400">
                          <p className="text-xs italic">{msg?.name}</p>
                          <p>{msg.msg}</p>
                        </div>
                      )}
